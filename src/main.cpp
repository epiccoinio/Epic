...
#if defined(NDEBUG)
#error "Epic cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uepic) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK())
        nSubsidy = 27000 * COIN;  // 5.4mil premine
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
   ...
   LogPrint("net", "received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->id);
    ...
    if (strCommand == "version") {
        ...
        // EPIC: We use certain sporks during IBD, so check to see if they are
